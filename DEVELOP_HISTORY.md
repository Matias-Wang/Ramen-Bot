...
### 修正
`「臺北市中山區」→ 0 筆`，而 `「中山區」→ 3 筆`。既有問題（已用 `git stash`
對照 HEAD 確認非前次引入）。根因是正規化只做了半邊：查詢被去掉「市/區/縣」
（臺北市中山區 → 臺北中山），店家 `location` 卻原封不動（臺北市中山區），
兩邊互不為子字串。

修法：抽出 `clean_location()`（去行政層級用字 + 「台/臺」正規化），
**查詢與店家 location 兩側都套用同一個函式**。順帶收斂了原本兩個分支各有一份
的重複清洗邏輯。

### 影響範圍已量測，不是憑感覺
對「真實日誌地區 + 全部店家 location + 各種寫法變體」的所有（查詢, 店家）配對
做改動前後比對：

- **新增比對成功 52 組，失去 0 組** —— 純粹是加分，無任何回歸。
- 52 組中絕大多數是「查詢 X 命中 X 自己」，即本次要修的 bug。
- 唯一需要細看的是「中山站」開始命中 location 為「中山區」的店家。查核後無害：
  該店（鳥人拉麵 中山店）**有座標**，捷運站查詢走 Haversine 不走字串比對；
  只有在座標查詢回 0 筆的 fallback 情境才會用到字串路徑，而那時
  「中山站查到中山區的店」本來就是正確行為。

驗證：`臺北市中山區`／`台北市中山區`／`臺北市大安區`／`高雄市左營區`／`臺中市西區`／

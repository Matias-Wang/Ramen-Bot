...
- **非阻塞處理**：`handle_message` 收到請求後立即啟動 `threading.Thread`，主執行緒直接返回 200，AI 邏輯在背景完成後以 `push_message` 回覆（取代有 60 秒過期限制的 `reply_message`）。啟動時預熱 Firestore、Gemini、Google Maps、Gemini Client Pool，確保首次請求也能快速回應。
  ...
  - ℹ️ **`min-instances` 已於 2026-09-02 由 1 改為 0**（成本考量：常駐實例讓 CPU
    24 小時空轉，全月 730 小時全額計費、與流量無關）。**`--no-cpu-throttling`
    不受影響、仍為硬性前提**。取捨：約 43% 的訊息會踩到冷啟動；另背景執行緒與
    實例回收之間新增一個低機率競態（webhook 回 200 後 Cloud Run 視實例為無活躍
    請求）。詳見 `review/review_20260902_0801.md`。
  - 📊 **冷啟動實測（2026-09-03）**：裸冷啟動 **6.4~12.3 秒**（非穩定值），
    其中模組 import 約 8.6s、連線預熱約 3.0s——**大頭是 import**，
    故將預熱改為背景非阻塞最多僅省約 3 秒。含 LLM 的完整體感：知識問答
    7.4s（熱）→ **13.9s**（冷）；快取命中的搜尋 2.15s（熱）→ 約 8.5s（冷）。
    15 分鐘閒置回收已由日誌證實。
  - 📐 **`[KPI][webhook]` 的計時起點**：取自 `timing.webhook_received_at()`，
    以 LINE 的 `event.timestamp` 為準，**故冷啟動（容器啟動 + 模組載入）亦計入**。
    不可改回 `time.time()`——該值在 `handle_message` 第一行取得，而該函式只在容器
    啟動完畢後才執行，`min-instances=0` 下會使 KPI 低報約 47%
    （2026-09-03 實測 13.87s 只報 7.34s，即為本修正的由來，PR #21）。
    時戳落差不在 0 ~ `MAX_WEBHOOK_LAG_S`（300s）之間時退回本機時間，
    以防時鐘偏移或 LINE 重送舊訊息造成 KPI 負值。

...
- **核心邏輯**：
    ...
    2. 地區比對（依查詢類型分流）：
       - 行政區查詢（地名以「區/市/縣」結尾）→ 跳過 Geocoding，直接以字串去除
         「市/區/縣」後比對 `shop["location"]`；比對前兩側皆以 `normalize_tai()`
         將「台北/台中/台南/台東/台灣」正規化為「臺」寫法——店家 `location` 是
         Places API 回寫的官方「臺」字，使用者卻幾乎都打「台」。
         **只處理這五個地名詞**：「台」在舞台／電台／平台／台階等詞裡並非「臺」
         的異體，全域替換會製造出錯誤的字。原因：行政區面積大且形狀不
